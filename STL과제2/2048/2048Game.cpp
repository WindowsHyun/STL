...
namespace std {
	//istream& operator >> ( istream& is, savePair& v ) {
	////	v.first = "";
	////	string temp;
...
LRESULT CALLBACK WndProc( HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM  lParam ) {
			...
			for ( auto d : loadData ) {
				...
				for ( int i = 0; d.first[i] != '\0'; ++i ) {
					if ( d.first[i] != ' ' ) {
						...
						cout << numData << endl;
						char *tempData = (char*)numData.c_str();
						gameData[y][x] = atoi( tempData );
						numData = "";
						++y;
						if ( y >= Game_Height ) {
							y = 0;
	...
	case WM_TIMER:
		...
		//if ( (dura.count() - 1 < give->first) &&
		//	(dura.count() + 1 > give->first) ) {
		//	SendMessage( hwnd, WM_CHAR, give->second, lParam );			//wm_char를 불러줌, 어떠한 값이 들어왔을 때의
		//	give++;
		//	iter++;
		//}
		break;
